...
const CreateRoom = (props: CreateRoomProps) => {
  const { step, nfts, tokens, collections, note, ...restProps } = props;
  ...
  const renderStepContent = () => {
    switch (step) {
      ...
      case "add-collection":
        return <LookingFor collections={collections} />;
      default:
        return <></>;
    }
  };
    ...
    <div className="w-full relative">
      <div className="h-full px-[52px] py-[64px] mr-[400px] 8xl:mr-[540px] 9xl:mr-[594px]">
        <div className="flex justify-center items-center w-full h-full relative">
          ...
          <div className="z-10 w-full h-full max-w-[876px] max-h-[556px] bg-gray-520 shadow-room rounded-[12px] border border-gray-25 px-[44px] py-[24px]">
            {renderStepContent()}
          </div>
        </div>
      </div>
      <div className="absolute right-0 top-0 bottom-0 w-[400px] 8xl:w-[540px] 9xl:w-[594px] bg-gray-550 z-10">
        <Sidebar {...restProps} step={step} notes={note} nfts={nfts} tokens={tokens} collections={collections} />
      </div>
    </div>
  );
};
